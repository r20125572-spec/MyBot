...
async def step2_create_pm(session, card_data, full_name, email, address_info, proxy):
    ...
    if "error" in rj:
        ...
        # Mapping exact Stripe API responses
...
async def step3_submit(session, cart_token, pm, full_name, email, crumb, xsrf, address_info, grand_total, proxy):
    ...
    # Mapping exact Order API responses
    if ss == "ORDER_CONFIRMED":
        return "CHARGED", "ORDER_CONFIRMED - Payment Successful"
    elif ft:
        return "DECLINED", f"DECLINED | {ft}"
    else:
        return "DECLINED", json.dumps(rj)[:150] # Show raw JSON if unknown decline
