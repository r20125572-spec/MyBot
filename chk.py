...
async def cmd_chk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.bot_data.get('chk_on', True):
        ...
        return
    if not context.args:
        await update.message.reply_text("❌ <code>/chk cc|mm|yy|cvv</code>", parse_mode="HTML")
        return
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            tasks = [session.get(STRIPE_API, params={"cc": card, "site": STRIPE_SITE}), get_bin_info(bin_num)]
