...
def parse_cards(text: str) -> list:
    ...
    for line in text.split("\n"):
        ...
        if len(parts) >= 4:
            ...
            if re.search(r"\d{13,19}", card_num): cards.append(line)
            continue
        match = re.search(r"(\d{13,19})\s*[|,;\s]\s*(\d{1,2})\s*[|,;\s]\s*(\d{2,4})\s*[|,;\s]\s*(\d{3,4})", line)
        if match:
        ...
        if match:
            ...
            if remaining:
                ...
                if len(extra) >= 3: cards.append(f"{card_num}|{extra[0]}|{extra[1]}|{extra[2]}")
                else: cards.append(card_num)
            else: cards.append(card_num)
    return cards
...
async def extract_cards_from_update(update: Update, bot, context: ContextTypes.DEFAULT_TYPE) -> list | None:
    msg = update.message
    if context.args:
...
async def process_mass(update: Update, context: ContextTypes.DEFAULT_TYPE, gate_key: str):
    ...
    if not is_premium:
        await update.message.reply_text("<b>⚠️ Pʀᴇᴍɪᴜᴍ Gᴀᴛᴇ</b>\n\nThis gate is only available for premium users.\nUse /plan to upgrade.", parse_mode="HTML")
        return

    cards = await extract_cards_from_update(update, context.bot, context)
    if not cards:
        await update.message.reply_text(f"⚠️ <b>Uꜱᴀɢᴇ:</b>\n\n1️⃣ Reply to a .txt file and send <code>/{gate_key}</code>\n2️⃣ Send a .txt file with <code>/{gate_key}</code> as caption\n3️⃣ <code>/{gate_key} cc|mm|yy|cvv</code>", parse_mode="HTML")
        return
    ...
    if not api_url:
        await update.message.reply_text("⚠️ Gate API not set.", parse_mode="HTML"); return

    rotator = ProxyRotator(PROXIES)
    dynamic_sites = SITES if SITES else [site]

    msg = await update.message.reply_text(f"[₪] <b>Gᴀᴛᴇ</b> ➺ {gate_name}\n━━━━━━━━━━━━━━\n      [◈] <b>Sᴛᴀᴛᴜs</b> ➺ Sᴛᴀʀᴛɪɴɢ...\n━━━━━━━━━━━━━━\n📊 <b>Cᴀʀᴅꜱ</b> ➺ {len(cards)}\n━━━━━━━━━━━━━━\n⏳ Pʀᴏᴄᴇꜱꜱɪɴɢ...", parse_mode="HTML")
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        ...
        for i, card in enumerate(cards):
            ...
            tasks.append(check_single_card(session, gate_key, api_url, site_to_use, card, proxy, semaphore))
