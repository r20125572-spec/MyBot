...
from config import (
    ...
    API_TIMEOUT, REFERRAL_CREDITS,
    GATE_URLS, GATE_SITES, PREMIUM_GATES, FORCE_CHANNELS,
    get_bin_info, kb_result,
)
from mass import get_mass_handlers  # Imports your separate mass.py file
...
def get_user_data(user_id: int, context: ContextTypes.DEFAULT_TYPE) -> dict:
    ...
    if uid not in context.bot_data["user_data"]:
        context.bot_data["user_data"][uid] = {
            "name": "User", "first_name": "User", "last_name": "", "username": "", "language_code": "en",
            "joined": datetime.now().strftime("%Y-%m-%d %H:%M"), "last_active": datetime.now().strftime("%Y-%m-%d %H:%M"),
            "credits": 150, "plan": "TRIAL", "expires": 0, "total_refs": 0, "total_checks": 0,
            "approved_checks": 0, "declined_checks": 0, "last_gate": "N/A", "last_card": "N/A",
...
def ui_profile(user, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    if premium and expires > now:
        ...
        if receipt: lines.append(f"Rᴇᴄᴇɪᴘᴛ ➺ <code>{receipt}</code>")
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def build_check_result(card_raw, gate_name, raw_response, bin_data, username, plan, time_taken, is_approved, is_timeout=False, is_error=False) -> str:
...
def kb_main(user_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(B("CHECKER"),  callback_data="mgates"), InlineKeyboardButton(B("BUY NOW"), callback_data="mprice")],
        [InlineKeyboardButton(B("UPDATES") + " ➺", url=CHANNEL_LINK), InlineKeyboardButton(B("GROUP")   + " ➺", url=GROUP_LINK)],
...
def kb_price() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(B("10$ - CORE"),  callback_data="pay10"), InlineKeyboardButton(B("15$ - ELITE"), callback_data="pay15"), InlineKeyboardButton(B("30$ - ROOT"),  callback_data="pay30")],
        [InlineKeyboardButton(B("SUPPORT") + " ➺", url=SUPPORT_LINK)],
...
def kb_gate_main() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(B("AUTH"),    callback_data="mauth"), InlineKeyboardButton(B("CHARGE"),  callback_data="mcharge"), InlineKeyboardButton("👑 " + B("PREMIUM"), callback_data="mmass")],
        [InlineKeyboardButton("🔙 " + B("BACK"), callback_data="bmain")],
    ])
...
def kb_charge_gates() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(B("STRIPE"),  callback_data="ichk"), InlineKeyboardButton(B("PAYPAL"),  callback_data="ipp")],
        [InlineKeyboardButton(B("SHOPIFY"), callback_data="ish"), InlineKeyboardButton(B("PAYU"),    callback_data="ipyu")],
        [InlineKeyboardButton("🔙 " + B("BACK"), callback_data="mgates")],
    ])
    ...
    try:
        await context.bot.send_message(
            ...
            text=(f"Rᴇꜰᴇʀʀᴀʟ Bᴏɴᴜꜱ\n━━━━━━━━━━━━━━━━━\nSomeone joined via your link!\n"
                  f"Cʀᴇᴅɪᴛꜱ Aᴅᴅᴇᴅ    ➺ +{REFERRAL_CREDITS}\nTᴏᴛᴀʟ Rᴇꜰᴇʀʀᴀʟꜱ ➺ {referrer_ud['total_refs']}\n━━━━━━━━━━━━━━━━━"),
        )
...
async def process_gate(update: Update, context: ContextTypes.DEFAULT_TYPE, gate_key: str, gate_name: str):
    ...
    if not premium:
        ...
        if credits <= 0:
            await update.message.reply_text("[ 𖥷iТ ] ➺ Nᴏ Cʀᴇᴅɪᴛꜱ\n━━━━━━━━━━━━━━━━━\nBuy a plan: /plan\nRefer friends for free credits: /refer", reply_markup=kb_upgrade()); return
...
async def cmd_bin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        txt = (
            ...
            f"Scheme  ➺ {str(bd.get('scheme', 'N/A')).upper()}\nType    ➺ {str(bd.get('type',   'N/A')).upper()}\n"
            f"Bank    ➺ {bd.get('bank', 'N/A')}\nCountry ➺ {bd.get('country_emoji', '')} {str(bd.get('country', 'N/A')).upper()}\n━━━━━━━━━━━━━━━━━"
        )
...
# FEEDBACK SYSTEM  (/fb)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def _fb_key(user_id: int) -> str: return f"{user_id}_{int(time.time())}_{random.randint(1000, 9999)}"
...
async def handle_fb_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.bot_data.setdefault("fb_pending", {})[key] = {
        "file_id": file_id, "file_type": file_type, "user_id": user.id, "username": uname,
        "name": user.full_name or user.first_name or "User", "note": user_note, "date": submitted,
    }
    ...
    try:
        if file_type == "photo": await context.bot.send_photo(chat_id=OWNER_ID, photo=file_id, caption=owner_caption, reply_markup=kb_fb_owner(key))
        else: await context.bot.send_video(chat_id=OWNER_ID, video=file_id, caption=owner_caption, reply_markup=kb_fb_owner(key))
...
async def _fb_approve(query, context: ContextTypes.DEFAULT_TYPE, key: str):
    ...
    posted = False
    try:
        if file_type == "photo": await context.bot.send_photo(chat_id=CHANNEL_USERNAME, photo=file_id, caption=channel_caption)
        else: await context.bot.send_video(chat_id=CHANNEL_USERNAME, video=file_id, caption=channel_caption)
        posted = True
...
async def cmd_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    txt += (
        f"━━━━━━━━━━━━━━━━━\nJᴏɪɴᴇᴅ      ➺ {udata.get('joined', 'N/A')}\nLᴀꜱᴛ Aᴄᴛɪᴠᴇ ➺ {udata.get('last_active', 'N/A')}\n━━━━━━━━━━━━━━━━━\n"
        f"Tᴏᴛᴀʟ Cʜᴇᴄᴋꜱ ➺ {total_checks}\nAᴘᴘʀᴏᴠᴇᴅ     ➺ {approved_checks}\nDᴇᴄʟɪɴᴇᴅ     ➺ {declined_checks}\nRᴀᴛᴇ         ➺ {approval_rate}\n"
        f"Lᴀꜱᴛ Gᴀᴛᴇ   ➺ {udata.get('last_gate', 'N/A')}\nLᴀꜱᴛ BIN    ➺ <code>{udata.get('last_card', 'N/A')}</code>\n━━━━━━━━━━━━━━━━━\n"
        f"Rᴇꜰᴇʀʀᴀʟꜱ   ➺ {total_refs}\nCᴏᴅᴇꜱ        ➺ {udata.get('codes_redeemed', 0)} redeemed\nKᴇʏꜱ         ➺ {udata.get('keys_redeemed', 0)} redeemed\n━━━━━━━━━━━━━━━━━"
    )
    if len(txt) > MAX_MSG:
...
async def cmd_find(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid_str, ud in all_users.items():
        if is_id:
            if uid_str == query:
                matches.append((uid_str, ud)); break
        else:
...
async def cmd_addcredits(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(chat_id=uid, text=(f"[ 𖥷iТ ] ➺ Cʀᴇᴅɪᴛꜱ Aᴅᴅᴇᴅ\n━━━━━━━━━━━━━━━━━\nAᴅᴅᴇᴅ   ➺ +{amt}\nBᴀʟᴀɴᴄᴇ ➺ {ud['credits']}\n━━━━━━━━━━━━━━━━━"))
    except Exception: pass
    await update.message.reply_text(
        f"━━━━━━━━━━━━━━━━━\n✅ Cʀᴇᴅɪᴛꜱ Aᴅᴅᴇᴅ\n━━━━━━━━━━━━━━━━━\nUꜱᴇʀ ➺ {uname_line}\nID ➺ <code>{uid}</code>\nAᴅᴅᴇᴅ ➺ +{amt}\nPʀᴇᴠ ➺ {old}\nNᴇᴡ ➺ {ud['credits']}\n━━━━━━━━━━━━━━━━━",
        parse_mode="HTML",
...
async def cmd_resub(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try: await context.bot.send_message(chat_id=uid, text="Your premium has been removed. Account reset to Trial.")
    except Exception: pass
...
async def cmd_allplans(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    all_users     = context.bot_data.get("user_data", {})
    now           = time.time()
    premium_users = [(u, d) for u, d in all_users.items() if d.get("plan", "TRIAL").upper() != "TRIAL" and d.get("expires", 0) > now]
    ...
    for uid_str, ud in premium_users:
        rem = ud["expires"] - now
        txt += (
            f"Nᴀᴍᴇ ➺ {ud.get('name', 'Unknown')}\nID   ➺ <code>{uid_str}</code>\nPʟᴀɴ ➺ {get_styled_plan(ud['plan'])}\nLᴇꜰᴛ ➺ {int(rem // 86400)}d {int((rem % 86400) // 3600)}h\n━━━━━━━━━━━━━━━━━\n"
        )
    if len(txt) > MAX_MSG:
        for i in range(0, len(txt), MAX_MSG): await update.message.reply_text(txt[i:i+MAX_MSG], parse_mode="HTML")
    else: await update.message.reply_text(txt, parse_mode="HTML")
...
async def cmd_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    broadcast_text = (
        f"[ 𖥷iТ ] Batman Announcement\n━━━━━━━━━━━━━━━━━\n{text}\n━━━━━━━━━━━━━━━━━\nPʀᴏ ➺ <a href='{BOT_LINK}'>Batman Card Checker</a>"
    )

    chan_ok = False
    try:
        await context.bot.send_message(chat_id=CHANNEL_USERNAME, text=broadcast_text, parse_mode="HTML", disable_web_page_preview=True)
        chan_ok = True
    except Exception as e: logger.warning(f"Broadcast channel failed: {e}")

    for uid_str in list(all_users.keys()):
        try:
            await context.bot.send_message(chat_id=int(uid_str), text=broadcast_text, parse_mode="HTML", disable_web_page_preview=True)
            sent += 1
        except Exception as e:
            err = str(e).lower()
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "check_sub":
        not_joined = await check_force_sub(user.id, context)
        if not_joined:
            try: await query.answer("❌ You haven't joined all channels/groups yet!\nJoin them and press Verify again.", show_alert=True)
            except Exception: pass
            try: await query.message.edit_reply_markup(reply_markup=kb_force_sub(not_joined))
            except Exception: pass
        else:
            try: await query.answer("✅ Verified! Welcome.", show_alert=False)
            except Exception: pass
            await edit(ui_profile(user, context), kb_main(user.id))
        return

    elif data == "bmain": await edit(ui_profile(user, context), kb_main(user.id))
    elif data == "mgates": await edit("[ 𖥷iТ ] Sᴇʟᴇᴄᴛ Gᴀᴛᴇ Cᴀᴛᴇɢᴏʀʏ\n━━━━━━━━━━━━━━━━━\nAᴜᴛʜ Gᴀᴛᴇꜱ   ➺ 1  (Free)\nCʜᴀʀɢᴇ Gᴀᴛᴇꜱ ➺ 4  (Free)\nPʀᴇᴍɪᴜᴍ Gᴀᴛᴇꜱ ➺ 3  (Premium)\n━━━━━━━━━━━━━━━━━", kb_gate_main())
    elif data == "mprice": await edit("[ 𖥷iТ ] Batman Premium Plans\n━━━━━━━━━━━━━━━━━\n\nAᴄᴄᴇꜱꜱ  ➺ Cᴏʀᴇ\nDᴀʏꜱ     ➺ 7\nCʀᴇᴅɪᴛꜱ ➺ Unlimited\nPʀɪᴄᴇ   ➺ 10$\n━━━━━━━━━━━━━━━━━\nAᴄᴄᴇꜱꜱ  ➺ Eʟɪᴛᴇ\nDᴀʏꜱ     ➺ 15\nCʀᴇᴅɪᴛꜱ ➺ Unlimited\nPʀɪᴄᴇ   ➺ 15$\n━━━━━━━━━━━━━━━━━\nAᴄᴄᴇꜱꜱ  ➺ Rᴏᴏᴛ\nDᴀʏꜱ     ➺ 30\nCʀᴇᴅɪᴛꜱ ➺ Unlimited\nPʀɪᴄᴇ   ➺ 30$\n━━━━━━━━━━━━━━━━━", kb_price())
    elif data == "mauth": await edit("[ 𖥷iТ ] Auth Gates (Free)\n━━━━━━━━━━━━━━━━━\n\nBʀᴀɪɴᴛʀᴇᴇ Aᴜᴛʜ ➺ /b3\n━━━━━━━━━━━━━━━━━", kb_auth_gates())
    elif data == "mcharge": await edit("[ 𖥷iТ ] Charge Gates (Free)\n━━━━━━━━━━━━━━━━━\n\nSᴛʀɪᴘᴇ  ➺ /chk\nPᴀʏPᴀʟ  ➺ /pp\nSʜᴏᴘɪꜰʏ ➺ /sh\nPᴀʏU    ➺ /pyu\n━━━━━━━━━━━━━━━━━", kb_charge_gates())
    elif data == "mmass":
        if not premium:
            try: await query.answer("Premium Gates require a premium plan!", show_alert=True)
            except Exception: pass
            await edit("[ 𖥷iТ ] Premium Gates\n━━━━━━━━━━━━━━━━━\n\nUpgrade: /plan", kb_upgrade()); return
        await edit("[ 𖥷iТ ] Premium Gates 👑\n━━━━━━━━━━━━━━━━━\n\nSᴛʀɪᴘᴇ Aᴜᴛʜ ➺ /au\nSᴛʀɪᴘᴇ Mᴀꜱꜱ ➺ /mss\nPᴀʏPᴀʟ Mᴀꜱꜱ ➺ /mpp2\n━━━━━━━━━━━━━━━━━", kb_premium_gates())
    elif data == "mrefer":
        link       = get_referral_link(user.id)
        total_refs = ud.get("total_refs", 0)
        await edit(f"[ 𖥷iТ ] Rᴇꜰᴇʀʀᴀʟ\n━━━━━━━━━━━━━━━━━\nLɪɴᴋ    ➺ <code>{link}</code>\n━━━━━━━━━━━━━━━━━\nRᴇꜰᴇʀʀᴀʟꜱ ➺ {total_refs}\nEᴀʀɴᴇᴅ ➺ {total_refs * REFERRAL_CREDITS} credits\nPᴇʀ Rᴇꜰ ➺ +{REFERRAL_CREDITS} credits\n━━━━━━━━━━━━━━━━━\nShare your link!", kb_back("bmain"))
    elif data == "iau":
        if not premium:
        ...
        if data.startswith("fb_ok_"): await _fb_approve(query, context, key)
        else: await _fb_decline(query, context, key)
...
def main():
    ...
    app.add_handler(MessageHandler((filters.PHOTO | filters.VIDEO) & filters.CaptionRegex(r"(?i)^/fb"), handle_fb_media))
