...
def ui_profile(user, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    if premium and expires > now:
        ...
        lines.append(f"Exᴘɪʀᴇꜱ ➺ {datetime.fromtimestamp(expires).strftime('%Y-%m-%d %H:%M')}")
        lines.append(f"Lᴇꜰᴛ    ➺ {rem_d}d {rem_h}h")
        if ud.get("last_receipt"):
            lines.append(f"Rᴇᴄᴇɪᴘᴛ ➺ <code>{ud['last_receipt']}</code>")
    lines.append(f"Rᴇꜰᴇʀʀᴀʟꜱ ➺ {total_refs} (+{total_refs * REFERRAL_CREDITS} credits)")
    lines.append(f"Jᴏɪɴᴇᴅ  ➺ {ud.get('joined', 'N/A')}")
...
def build_check_result(card_raw, gate_name, raw_response, bin_data,
    ...
    return "\n".join([
        ...
        f"Rᴀᴡ  ➺ {raw_response}",
        f"Iɴꜰᴏ ➺ {bin_txt}",
        f"Uꜱᴇʀ ➺ {username} {get_plan_icon(plan)} ({get_styled_plan(plan)})",
...
async def process_gate(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    if not api_url:
        await update.message.reply_text(
            f"Gate API not set. Use: /seturl {gate_key} &lt;url&gt;", parse_mode="HTML")
        return
    msg        = await update.message.reply_text(f"{TAG} ➺ Sᴄᴀɴɴɪɴɢ...")
    ...
    try:
        ...
        text = build_check_result(card_raw, gate_name, raw_response, bin_data,
                                   uname, plan, f"{time.time()-start_time:.2f}", is_approved)
        await msg.edit_text(text, parse_mode="HTML",
                            reply_markup=kb_result(), disable_web_page_preview=True)
    except asyncio.TimeoutError:
        if not premium: ud["credits"] = ud.get("credits", 0) + 1
        text = build_check_result(card_raw, gate_name, "Rᴇǫᴜᴇꜱᴛ Tɪᴍᴇᴏᴜᴛ", {},
                                   uname, plan, f"{time.time()-start_time:.2f}",
                                   False, is_timeout=True)
        await msg.edit_text(text, parse_mode="HTML",
                            reply_markup=kb_result(), disable_web_page_preview=True)
...
async def send_activation_msg(user_id: int, plan: str, days: int,
    ...
    txt = (
        f"{TAG} ➺ Aᴄᴄᴇꜱꜱ Aᴄᴛɪᴠᴀᴛᴇᴅ\n━━━━━━━━━━━━━━━━━\n"
        f"Uꜱᴇʀ     ➺ {display}\n"
        f"Aᴄᴄᴇꜱꜱ  ➺ {get_styled_plan(plan)} 👑\n"
        f"Dᴀʏꜱ    ➺ {days}\nCʀᴇᴅɪᴛꜱ ➺ Unlimited\n"
        f"Exᴘɪʀᴇꜱ ➺ {datetime.fromtimestamp(expires_ts).strftime('%Y-%m-%d %H:%M')}\n"
...
async def cmd_ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    t   = time.time()
    msg = await update.message.reply_text(f"{TAG} ➺ Pɪɴɢɪɴɢ...")
    await msg.edit_text(f"{TAG} ➺ Pᴏɴɢ | {int((time.time()-t)*1000)}ms")

async def cmd_plan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"{TAG} Batman Premium Plans\n━━━━━━━━━━━━━━━━━\n\n"
        "Aᴄᴄᴇꜱꜱ  ➺ Cᴏʀᴇ\nDᴀʏꜱ     ➺ 7\nCʀᴇᴅɪᴛꜱ ➺ Unlimited\nPʀɪᴄᴇ   ➺ 10$\n"
        "━━━━━━━━━━━━━━━━━\n"
        "Aᴄᴄᴇꜱꜱ  ➺ Eʟɪᴛᴇ\nDᴀʏꜱ     ➺ 15\nCʀᴇᴅɪᴛꜱ ➺ Unlimited\nPʀɪᴄᴇ   ➺ 15$\n"
        "━━━━━━━━━━━━━━━━━\n"
...
async def cmd_bin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not bin_num or not bin_num.isdigit() or len(bin_num) < 6:
        ...
        return
    msg = await update.message.reply_text(f"{TAG} ➺ Lᴏᴏᴋɪɴɢ ᴜᴘ...")
...
async def cmd_fb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await msg.reply_text(
        "━━━━━━━━━━━━━━━━━\n📸 Fᴇᴇᴅʙᴀᴄᴋ\n━━━━━━━━━━━━━━━━━\n\n"
        "Sᴇɴᴅ a photo or video WITH caption /fb\n\n"
...
async def handle_fb_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cap = (f"{TAG} ➺ Nᴇᴡ Fᴇᴇᴅʙᴀᴄᴋ\n━━━━━━━━━━━━━━━━━\n"
           f"Uꜱᴇʀ ➺ {uname}\nID   ➺ {user.id}\n"
           f"Dᴀᴛᴇ ➺ {submitted}\nTʏᴘᴇ ➺ {file_type.capitalize()}\n")
    if user_note:
        cap += f"Nᴏᴛᴇ ➺ {user_note[:200]}\n"
...
async def _fb_approve(query, context: ContextTypes.DEFAULT_TYPE, key: str):
    ...
    file_id, file_type = fb["file_id"], fb["file_type"]
    user_note, submitted = fb.get("note",""), fb["date"]
    channel_cap = "━━━━━━━━━━━━━━━━━\n"
    if user_note:
        channel_cap += f"{user_note}\n━━━━━━━━━━━━━━━━━\n"
    channel_cap += f"Uꜱᴇʀ ➺ {uname}\nID   ➺ {uid}\nDᴀᴛᴇ ➺ {submitted}\n━━━━━━━━━━━━━━━━━"
    posted = False
    try:
        send  = context.bot.send_photo if file_type == "photo" else context.bot.send_video
        kwarg = {"photo": file_id} if file_type == "photo" else {"video": file_id}
        ...
        try:
            await context.bot.send_message(
                chat_id=OWNER_ID,
                text=(f"⚠️ Channel post failed!\n"
                      f"Error: <code>{str(e)[:300]}</code>\n\n"
                      f"Make sure bot is admin in channel with Post Messages permission.\n"
                      f"CHANNEL_ID used: <code>{CHANNEL_ID}</code>"),
                parse_mode="HTML")
        except Exception: pass
    context.bot_data["fb_pending"].pop(key, None)
    status_line = "✅ Posted to channel!" if posted else "⚠️ Approved but channel post failed."
    try:
        await query.message.edit_caption(
            caption=(f"{TAG} ➺ Fᴇᴇᴅʙᴀᴄᴋ Aᴘᴘʀᴏᴠᴇᴅ\n━━━━━━━━━━━━━━━━━\n"
                     f"Uꜱᴇʀ   ➺ {uname}\nID     ➺ {uid}\nSᴛᴀᴛᴜꜱ ➺ {status_line}\n━━━━━━━━━━━━━━━━━"),
            reply_markup=None)
    except Exception: pass
    try:
        await context.bot.send_message(
            chat_id=uid,
            text=(f"{TAG} ➺ Fᴇᴇᴅʙᴀᴄᴋ Aᴄᴄᴇᴘᴛᴇᴅ ✅\n━━━━━━━━━━━━━━━━━\n"
                  f"Your feedback was posted to the channel!\n📢 {CHANNEL_LINK}\n━━━━━━━━━━━━━━━━━"))
    except Exception: pass
...
async def _fb_decline(query, context: ContextTypes.DEFAULT_TYPE, key: str):
    ...
    try:
        await query.message.edit_caption(
            caption=(f"{TAG} ➺ Fᴇᴇᴅʙᴀᴄᴋ Dᴇᴄʟɪɴᴇᴅ\n━━━━━━━━━━━━━━━━━\n"
                     f"Uꜱᴇʀ   ➺ {uname}\nID     ➺ {uid}\nSᴛᴀᴛᴜꜱ ➺ ❌ Declined\n━━━━━━━━━━━━━━━━━"),
            reply_markup=None)
    except Exception: pass
    try:
...
async def cmd_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not target_id:
        await msg.reply_text(
            "❌ User not found.\n\nUsage:\n/info @username\n/info 123456789\n"
            "/info John\nOr reply to any message → /info")
        return
    uid_str   = str(target_id)
    all_users = context.bot_data.get("user_data", {})
    udata     = all_users.get(uid_str, {})
    now       = time.time()
    first   = getattr(target_chat, "first_name",  None) or udata.get("first_name", "Unknown")
    last    = getattr(target_chat, "last_name",   None) or udata.get("last_name",  "")
