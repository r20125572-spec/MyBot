...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# RESULT KEYBOARD — BUY NOW / CHANNEL BUTTON
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# FORCE SUBSCRIBE (5-MIN CACHE — BOTH DIRECTIONS)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
async def check_force_sub(user_id: int, context: ContextTypes.DEFAULT_TYPE) -> list:
    ...
    _force_sub_cache[user_id] = {"not_joined": not_joined, "ts": time.time()}
    return not_joined
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def build_check_result(card_raw, gate_name, raw_response, bin_data, username, plan,
                       time_taken, is_approved, is_timeout=False, is_error=False) -> str:
...
async def process_gate(update: Update, context: ContextTypes.DEFAULT_TYPE, gate_key: str, gate_name: str):
    ...
    if not api_url:
        await update.message.reply_text("Gate API not configured.", parse_mode="HTML"); return
    ...
    try:
        ...
        text = build_check_result(
            ...
            bin_data=bin_data, username=uname, plan=plan, time_taken=time_taken,
        ...
        time_taken = f"{time.time() - start_time:.2f}"
        text = build_check_result(
            card_raw=card_raw, gate_name=gate_name, raw_response="Rᴇǫᴜᴇꜱᴛ Tɪᴍᴇᴏᴜᴛ",
            bin_data={}, username=uname, plan=plan, time_taken=time_taken,
        ...
        time_taken = f"{time.time() - start_time:.2f}"
        text = build_check_result(
            card_raw=card_raw, gate_name=gate_name, raw_response=str(e)[:120],
            bin_data={}, username=uname, plan=plan, time_taken=time_taken,
...
async def _gate_toggle(update, context, gate: str, state: bool):
    ...
    await update.message.reply_text(f"Gate [{gate.upper()}] {'ON ✅' if state else 'OFF ❌'}.")
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not_joined:
        try:
            await update.message.reply_photo(
                photo=BOT_PHOTO_URL, caption=FORCE_SUB_TEXT,
                reply_markup=kb_force_sub(not_joined), parse_mode="HTML"
            )
        except Exception:
            await update.message.reply_text(
                FORCE_SUB_TEXT, reply_markup=kb_force_sub(not_joined), parse_mode="HTML"
    ...
    try:
        await update.message.reply_photo(
            photo=BOT_PHOTO_URL, caption=ui_profile(user, context),
            parse_mode="HTML", reply_markup=kb_main(user.id)
        )
    except Exception:
        await update.message.reply_text(
...
async def _fb_approve(query, context: ContextTypes.DEFAULT_TYPE, key: str):
    ...
    uname, uid, submitted, file_id, file_type, user_note = (
        fb["username"], fb["user_id"], fb["date"], fb["file_id"], fb["file_type"], fb.get("note", "")
    )
    channel_caption = "━━━━━━━━━━━━━━━━━\n"
    ...
    try:
        if file_type == "photo":
            await context.bot.send_photo(chat_id=CHANNEL_USERNAME, photo=file_id, caption=channel_caption, parse_mode="HTML")
        else:
            await context.bot.send_video(chat_id=CHANNEL_USERNAME, video=file_id, caption=channel_caption, parse_mode="HTML")
        posted = True
    except Exception as e:
    ...
    try:
        await query.message.edit_caption(
            caption=f"<b>[ 𖥷iТ ] ➺ Fᴇᴇᴅʙᴀᴄᴋ Aᴘᴘʀᴏᴠᴇᴅ</b>\n━━━━━━━━━━━━━━━━━\n{'✅ Posted' if posted else '⚠️ Post failed'}\n━━━━━━━━━━━━━━━━━",
    ...
    try:
        await context.bot.send_message(
            chat_id=uid,
            text=f"<b>[ 𖥷iТ ] ➺ Fᴇᴇᴅʙᴀᴄᴋ Aᴄᴄᴇᴘᴛᴇᴅ ✅</b>\n━━━━━━━━━━━━━━━━━\nPosted to channel!\n📢 {CHANNEL_LINK}\n━━━━━━━━━━━━━━━━━",
            parse_mode="HTML"
        )
    except Exception:
        pass
...
async def cmd_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.reply_to_message and update.message.reply_to_message.from_user:
        ...
        target_id, target_name, target_last_name, target_username, target_lang = (
            ru.id, ru.first_name or "N/A", ru.last_name or "", ru.username, ru.language_code or "N/A"
        )
    elif context.args:
        ...
        if raw.lstrip("-").isdigit():
            ...
            try:
                ...
                target_id, target_name, target_last_name, target_username = (
                    chat.id, chat.first_name or "N/A", getattr(chat, "last_name", "") or "", chat.username
                )
            except Exception:
                pass
            if not target_id:
                ...
                for uid_str, ud in context.bot_data.get("user_data", {}).items():
                    if raw_lower in ud.get("username", "").lower().lstrip("@") or raw_lower in ud.get("name", "").lower():
                        target_id, target_name, target_username, target_lang = (
                            int(uid_str), ud.get("name", "N/A"), ud.get("username"), ud.get("language_code", "N/A")
                        )
                        break
    ...
    total_refs, total_checks, approved_checks, declined_checks = (
        udata.get("total_refs", 0), udata.get("total_checks", 0),
        udata.get("approved_checks", 0), udata.get("declined_checks", 0)
    )
    ...
    txt = (
        ...
        f"<b>Sᴛᴀᴛᴜꜱ</b>    ➺ {'Active ✅' if premium else 'Trial'}\n"
    )
    if premium and expires > now:
        rem = expires - now
        txt += (
...
async def cmd_find(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid_str, ud in matches[:10]:
        ...
        blocks.append(
            f"<b>Nᴀᴍᴇ</b> ➺ {ud.get('name','Unknown')}\n"
...
async def cmd_allplans(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    now           = time.time()
    premium_users = [
        (uid_str, ud) for uid_str, ud in context.bot_data.get("user_data", {}).items()
...
async def cmd_seturl(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ Updated {gate} URL:\n<code>{url}</code>", parse_mode="HTML")
...
async def cmd_geturl(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for gate in GATE_URLS:
        ...
        txt += f"{gate.upper()} ➺ <code>{url or 'Not Set'}</code>\n"
    await update.message.reply_text(txt, parse_mode="HTML")
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# CALLBACK QUERY HANDLER — ALL BUTTONS FULLY WORKING
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Smart editor: handles both photo captions and plain text messages
    async def edit(text: str, markup=None):
        try:
            ...
            if "message is not modified" not in str(e).lower():
                logger.warning(f"Callback edit: {e}")
        except Exception as e:
            logger.warning(f"Callback edit: {e}")
    ...
    if data == "check_sub":
        # Force-clear cache so we re-check right now
        _force_sub_cache.pop(user.id, None)
        not_joined = await check_force_sub(user.id, context)
        if not_joined:
            await edit(FORCE_SUB_TEXT, markup=kb_force_sub(not_joined))
        else:
            _update_user_meta(ud, user)
            await edit(ui_profile(user, context), markup=kb_main(user.id))
        return
    ...
    if data == "bmain":
        ...
        await edit("<b>[ 𖥷iТ ] ➺ Sᴇʟᴇᴄᴛ Gᴀᴛᴇ</b>\n━━━━━━━━━━━━━━━━━", markup=kb_gate_main())
    elif data == "mauth":
        await edit("<b>Aᴜᴛʜ Gᴀᴛᴇs</b>\n━━━━━━━━━━━━━━━━━", markup=kb_auth_gates())
    elif data == "mcharge":
        await edit("<b>Cʜᴀʀɢᴇ Gᴀᴛᴇs</b>\n━━━━━━━━━━━━━━━━━", markup=kb_charge_gates())
    elif data == "mmass":
        if not premium:
            await query.answer("Premium Gates require a premium plan!", show_alert=True)
            await edit(
                "<b>[ 𖥷iТ ] ➺ Pʀᴇᴍɪᴜᴍ Rᴇǫᴜɪʀᴇᴅ</b>\n━━━━━━━━━━━━━━━━━\n"
                "Upgrade with /plan to access premium gates.",
                markup=kb_upgrade()
            )
        else:
            await edit("<b>Pʀᴇᴍɪᴜᴍ Gᴀᴛᴇs 👑</b>\n━━━━━━━━━━━━━━━━━", markup=kb_premium_gates())
    ...
    # ── Buy Now / Pricing ──
        ...
        await edit(txt, markup=kb_price())

    # ── Payment tiers ──
    elif data.startswith("pay"):
        plan_map = {"pay10": "Core — 10$", "pay15": "Elite — 15$", "pay30": "Root — 30$"}
        plan_label = plan_map.get(data, "Selected Plan")
        await edit(
            f"<b>[ 𖥷iТ ] ➺ Pᴀʏᴍᴇɴᴛ</b>\n━━━━━━━━━━━━━━━━━\n"
            f"<b>Pʟᴀɴ</b>    ➺ {plan_label}\n\n"
            f"Contact support to complete your purchase.\n━━━━━━━━━━━━━━━━━",
            markup=kb_payment()
        )
    ...
    # ── Referral ──
        ...
        await edit(txt, markup=kb_back("bmain"))

    # ── Gate info buttons (i<gate>) ──
        ...
        if data in gate_map:
            ...
            if g_key in PREMIUM_GATES and not premium:
                await query.answer("This gate is Premium Only! Upgrade with /plan", show_alert=True)
                return
            await edit(gate_info_text(g_name, g_key, 1), markup=kb_back("mgates"))

    # ── Feedback approval (owner only) ──
    elif data.startswith("fb_ok_"):
        if user.id != OWNER_ID:
            await query.answer("Not authorized.", show_alert=True); return
        await _fb_approve(query, context, data[len("fb_ok_"):])
    elif data.startswith("fb_no_"):
        if user.id != OWNER_ID:
            await query.answer("Not authorized.", show_alert=True); return
        await _fb_decline(query, context, data[len("fb_no_"):])
...
async def global_error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if isinstance(error, Conflict):
        logger.critical("CONFLICT: Another bot instance running. Waiting 10s before exit...")
...
def main():
    ...
    if not acquire_instance_lock():
        logger.warning("Another instance is already running. Exiting."); return
    ...
    try:
        ...
        app.add_handler(get_b3_handler())          # b3.py — uses its own dedicated API

        # ── Mass Handlers (premium) + result download callbacks ──
