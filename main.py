...
GATE_COST = {"chk": 1, "pp": 1, "sh": 2, "pyu": 1, "b3": 1, "mss": 2, "mpp2": 2}
GATE_NAMES = {
    "chk": "Stripe Charge",
    "pp": "PayPal Charge",
    "sh": "Shopify Charge",
...
def kb_price():
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("💳 10$ CORE", callback_data="pay10"),
            InlineKeyboardButton("⭐ 15$ ELITE", callback_data="pay15"),
            InlineKeyboardButton("👑 30$ ROOT", callback_data="pay30"),
        ],
...
# ── FIXED aiohttp — json() called INSIDE session ──
...
async def process_gate(update: Update, context: ContextTypes.DEFAULT_TYPE, gate_key: str, gate_name: str):
    ...
    if not is_premium:
        ...
        if credits <= 0:
            await update.message.reply_text(
                "❌ Yᴏᴜʀ ᴛʀɪᴀʟ ᴄʀᴇᴅɪᴛꜱ ᴀʀᴇ ᴇᴍᴘᴛʏ.\n"
                "Pᴜʀᴄʜᴀꜱᴇ ᴀ ᴘʟᴀɴ ᴡɪᴛʜ /plan ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ.",
                parse_mode="HTML",
            )
            return
    ...
    if not api_url:
        await msg.edit_text("❌ Eʀʀᴏʀ ➤ Gate API URL not set. Ask owner to use /seturl.", parse_mode="HTML")
        return
    try:
        ...
        text = (
            ...
            f"Rᴀᴡ ➺ {raw_response}\n"
            f"Iɴꜰᴏ ➺ {bin_txt}\n"
            f"Uꜱᴇʀ ➺ {first_name} 👑 ({plan_ui})\n"
            f"Pʀᴏ ➺ Batman ⚡\n"
            f"Tɪᴍᴇ ➺ {time_taken}s"
        )
        await msg.edit_text(text, parse_mode="HTML", reply_markup=kb_result(), disable_web_page_preview=True)
    except aiohttp.ServerTimeoutError:
        await msg.edit_text("⏳ Tɪᴍᴇᴏᴜᴛ — Gᴀᴛᴇ ᴅɪᴅ ɴᴏᴛ ʀᴇꜱᴘᴏɴᴅ ɪɴ ᴛɪᴍᴇ.", parse_mode="HTML")
    except Exception as e:
        logger.error(f"process_gate error [{gate_key}]: {e}")
        await msg.edit_text(f"❌ Eʀʀᴏʀ ➤ <code>{str(e)[:120]}</code>", parse_mode="HTML")

# ── GATE COMMANDS ──
async def cmd_chk(update, context): await process_gate(update, context, "chk", "Sᴛʀɪᴘᴇ Cʜᴀʀɢᴇ | 1 ᴄʀᴇᴅɪᴛ")
async def cmd_pp(update, context):  await process_gate(update, context, "pp",  "PᴀʏPᴀʟ Cʜᴀʀɢᴇ | 1 ᴄʀᴇᴅɪᴛ")
async def cmd_sh(update, context):  await process_gate(update, context, "sh",  "Sʜᴏᴘɪꜰʏ Cʜᴀʀɢᴇ | 2 ᴄʀᴇᴅɪᴛs")
async def cmd_pyu(update, context): await process_gate(update, context, "pyu", "PᴀʏU Cʜᴀʀɢᴇ | 1 ᴄʀᴇᴅɪᴛ")
async def cmd_b3(update, context):  await process_gate(update, context, "b3",  "Bʀᴀɪɴᴛʀᴇᴇ Aᴜᴛʜ | 1 ᴄʀᴇᴅɪᴛ")
async def cmd_au(update, context):  await process_gate(update, context, "au",  "Sᴛʀɪᴘᴇ Aᴜᴛʜ | 1 ᴄʀᴇᴅɪᴛ")
async def cmd_mss(update, context): await process_gate(update, context, "mss", "Sᴛʀɪᴘᴇ Mᴀss | 2 ᴄʀᴇᴅɪᴛs")
async def cmd_mpp2(update, context):await process_gate(update, context, "mpp2","PᴀʏPᴀʟ Mᴀss | 2 ᴄʀᴇᴅɪᴛs")

async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    ud = get_user_data(user.id, context)
    if "joined" not in ud: ud["joined"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    if "name" not in ud: ud["name"] = user.first_name or "User"
    if await is_joined(user.id, context):
        ...
        caption = ("🦇 BATMAN CARD CHECKER 🦇\n\nAᴄᴄᴇꜱꜱ Rᴇǫᴜɪʀᴇᴅ\n━━━━━━━━━━━━━━━━━━━━\nJᴏɪɴ ʙᴏᴛʜ ᴄʜᴀɴɴᴇʟs ᴛᴏ ᴜɴʟᴏᴄᴋ ᴛʜᴇ ʙᴏᴛ.\n━━━━━━━━━━━━━━━━━━━━")
        try:
...
async def cmd_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.reply_to_message and not context.args:
        await update.message.reply_text("❌ Usage: /info <user_id>", parse_mode="HTML"); return
...
async def cmd_gen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except ValueError: await update.message.reply_text("❌ Invalid amount.", parse_mode="HTML")
...
async def cmd_resub(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    uid = await resolve_user(context.args[0], context)
    if not uid: await update.message.reply_text("❌ User not found.", parse_mode="HTML"); return
    ud = get_user_data(uid, context); ud["plan"] = "TRIAL"; ud["expires"] = 0
...
async def cmd_delcode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    code = context.args[0]; codes = context.bot_data.get("codes", {}); keys = context.bot_data.get("keys", {})
...
async def cmd_rm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    code = context.args[0].upper(); uid = update.effective_user.id
    ud = get_user_data(uid, context)
    ...
    else: await update.message.reply_text("❌ Invalid or already used code.", parse_mode="HTML")
...
def _make_gate_toggle(gate: str, label: str, state: bool):
    async def handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(f"{label} → {'ON ✅' if state else 'OFF ❌'}", parse_mode="HTML")
    return handler
...
async def cmd_seturl(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if url.lower() == "remove": context.bot_data.pop(f"gate_url_{gate}", None); await update.message.reply_text(f"✅ Removed {GATE_NAMES[gate]} URL.", parse_mode="HTML"); return
    context.bot_data[f"gate_url_{gate}"] = url
    await update.message.reply_text(f"✅ {GATE_NAMES[gate]} URL set.\n<code>{url}</code>", parse_mode="HTML")
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.bot_data.get("maintenance", False) and q.from_user.id != OWNER_ID:
        await q.answer("🛑 Bot is under maintenance.", show_alert=True); return

    async def safe_edit(text: str, markup):
        if not q.message: return
        try:
            if q.message.photo:
                await context.bot.send_message(chat_id=q.message.chat_id, text=text, parse_mode="HTML", reply_markup=markup, disable_web_page_preview=True)
            else:
                await q.edit_message_text(text=text, parse_mode="HTML", reply_markup=markup, disable_web_page_preview=True)
        except Exception as e:
            logger.warning(f"safe_edit: {e}")
            try: await context.bot.send_message(chat_id=q.message.chat_id, text=text, parse_mode="HTML", reply_markup=markup, disable_web_page_preview=True)
            except: pass

    if d == "verify_join":
        if await is_joined(q.from_user.id, context): await safe_edit(ui_profile(q.from_user, context), kb_main())
        else: await q.answer("❌ Join Group & Channel first!", show_alert=True)
    elif d == "bmain":   await safe_edit(ui_profile(q.from_user, context), kb_main())
    elif d == "mprice":  await safe_edit(PLAN_TEXT, kb_price())
    elif d == "pay10":   await safe_edit("━━━━━━━━━━━━━━━━━━━━\n💳 PAYMENT — 10$ (Cᴏʀᴇ | 7 Dᴀʏꜱ)\n━━━━━━━━━━━━━━━━━━━━\n\nCᴏɴᴛᴀᴄᴛ ꜱᴜᴘᴘᴏʀᴛ ᴛᴏ ᴄᴏᴍᴘʟᴇᴛᴇ ʏᴏᴜʀ ᴘᴀʏᴍᴇɴᴛ.\n━━━━━━━━━━━━━━━━━━━━", kb_payment())
    elif d == "pay15":   await safe_edit("━━━━━━━━━━━━━━━━━━━━\n⭐ PAYMENT — 15$ (Eʟɪᴛᴇ | 15 Dᴀʏꜱ)\n━━━━━━━━━━━━━━━━━━━━\n\nCᴏɴᴛᴀᴄᴛ ꜱᴜᴘᴘᴏʀᴛ ᴛᴏ ᴄᴏᴍᴘʟᴇᴛᴇ ʏᴏᴜʀ ᴘᴀʏᴍᴇɴᴛ.\n━━━━━━━━━━━━━━━━━━━━", kb_payment())
    elif d == "pay30":   await safe_edit("━━━━━━━━━━━━━━━━━━━━\n👑 PAYMENT — 30$ (Rᴏᴏᴛ | 30 Dᴀʏꜱ)\n━━━━━━━━━━━━━━━━━━━━\n\nCᴏɴᴛᴀᴄᴛ ꜱᴜᴘᴘᴏʀᴛ ᴛᴏ ᴄᴏᴍᴘʟᴇᴛᴇ ʏᴏᴜʀ ᴘᴀʏᴍᴇɴᴛ.\n━━━━━━━━━━━━━━━━━━━━", kb_payment())
    elif d == "mgates":  await safe_edit(CHECKER_STATUS_TEXT, kb_gate_main())
    elif d == "mauth":   await safe_edit("🔐 Sᴇʟᴇᴄᴛ ᴀɴ Aᴜᴛʜ Gᴀᴛᴇ ➺", kb_auth_gates())
    elif d == "mcharge": await safe_edit("💰 Sᴇʟᴇᴄᴛ ᴀ Cʜᴀʀɢᴇ Gᴀᴛᴇ ➺", kb_charge_gates())
    elif d == "mmass":   await safe_edit("🚀 Sᴇʟᴇᴄᴛ ᴀ Mᴀss Gᴀᴛᴇ ➺", kb_mass_gates())
    elif d == "iau":     await safe_edit(gate_info_text("Stripe Auth",      "au",   1), kb_back("mauth"))
    elif d == "ib3":     await safe_edit(gate_info_text("Braintree Auth",   "b3",   1), kb_back("mauth"))
    elif d == "ichk":    await safe_edit(gate_info_text("Stripe Charge",    "chk",  1), kb_back("mcharge"))
    elif d == "ipp":     await safe_edit(gate_info_text("PayPal Charge",    "pp",   1), kb_back("mcharge"))
    elif d == "ish":     await safe_edit(gate_info_text("Shopify Charge",   "sh",   2), kb_back("mcharge"))
    elif d == "ipyu":    await safe_edit(gate_info_text("PayU Charge",      "pyu",  1), kb_back("mcharge"))
    elif d == "imss":    await safe_edit(gate_info_text("Stripe Mass",      "mss",  2), kb_back("mmass"))
    elif d == "impp2":   await safe_edit(gate_info_text("PayPal Mass",      "mpp2", 2), kb_back("mmass"))
