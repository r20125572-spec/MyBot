...
def gate_charge_info(gate_name: str, cmd: str, cost: int) -> str:
    return (
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"{gate_name}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n\n"
...
async def anti_ad_filter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text and any(w in text for w in BLOCK_WORDS):
        try:
            await msg.delete()
        except Exception:
            pass
...
async def process_gate(update: Update, context: ContextTypes.DEFAULT_TYPE, gate_key: str, gate_name: str):
    ...
    if not api_url:
        await msg.edit_text(
            f"Gate API URL not set.\nOwner must run: /seturl {gate_key} &lt;url&gt;",
            parse_mode="HTML",
        )
        return
    try:
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_joined(user.id, context):
        ...
        caption = (
            ...
            "Access Required\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "Join both the channel and group to unlock the bot.\n"
            "━━━━━━━━━━━━━━━━━━━━"
        )
        try:
            await update.message.reply_photo(
                photo=WELCOME_IMAGE_URL, caption=caption,
                parse_mode="HTML", reply_markup=kb_force(),
            )
        except Exception:
            await update.message.reply_text(
                caption, parse_mode="HTML", reply_markup=kb_force(),
            )
...
async def cmd_bin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    txt = (
        ...
        f"𝗧𝘆𝗽𝗲 ➛ {card_type}\n"
        f"𝗖𝘂𝗿𝗿𝗲𝗻𝗰𝘆 ➛ {currency}\n"
        f"𝗨𝘀𝗲𝗿 ➛ {user.first_name or 'User'} ({get_styled_plan(raw_plan)})\n"
        f"𝗗𝗲𝘃 ➛ Batman"
    )
    try:
        await status.edit_text(
            txt, parse_mode="HTML",
            reply_markup=kb_bin_result(), disable_web_page_preview=True,
        )
    except Exception:
        pass
...
async def cmd_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        name, username, uid = u.first_name or "N/A", u.username or "None", u.id
    except Exception:
        pass
...
async def cmd_seturl(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if gate not in GATE_NAMES:
        await update.message.reply_text(
            f"Unknown gate: {gate}\nValid: chk pp sh pyu b3 au mss mpp2", parse_mode="HTML"
        )
        return
    if url.lower() == "remove":
...
def _toggle(gate: str, label: str, state: bool):
    async def handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            f"{label} — {'ON' if state else 'OFF'}", parse_mode="HTML"
        )
    return handler
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            if q.message.photo:
                await context.bot.send_message(
                    chat_id=q.message.chat_id, text=text,
    ...
    if d == "verify_join":
        ...
        await safe_edit(
            ...
            + PAYMENT_PENDING_TEXT, kb_payment()
        )
    elif d == "pay15":
        await safe_edit(
        ...
        await safe_edit(gate_charge_info("Stripe Charge",  "chk",  1), kb_back("mcharge"))
    elif d == "ipp":
        await safe_edit(gate_charge_info("PayPal Charge",  "pp",   1), kb_back("mcharge"))
    elif d == "ish":
        await safe_edit(gate_charge_info("Shopify Charge", "sh",   2), kb_back("mcharge"))
    elif d == "ipyu":
        await safe_edit(gate_charge_info("PayU Charge",    "pyu",  1), kb_back("mcharge"))
    elif d == "mmass":
        await safe_edit(MASS_MENU_TEXT, kb_mass_gates())
    elif d == "imss":
        await safe_edit(gate_charge_info("Stripe Mass",  "mss",  2), kb_back("mmass"))
    elif d == "impp2":
        await safe_edit(gate_charge_info("PayPal Mass",  "mpp2", 2), kb_back("mmass"))
