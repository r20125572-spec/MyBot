...
async def cmd_pp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.bot_data.get("pp_on", True):
        await update.message.reply_text("⚠️ Pᴀʏᴘᴀʟ gate is currently <b>OFF</b>.", parse_mode="HTML")
        return
    ...
    if not card:
        await update.message.reply_text(
            ...
            "<b>Example:</b>\n"
            "<code>/pp 4111111111111111|12|26|123</code>",
            parse_mode="HTML"
        )
        return
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            tasks = [
                session.get(api_url, headers={"User-Agent": "Mozilla/5.0", "Accept": "application/json"}), 
        ...
        # Extract exact new JSON response fields safely
        status    = str(data.get("status", "declined")).lower()
        message   = str(data.get("message") or data.get("code") or "NO RESPONSE").strip()
        
        # ── STRICT STATUS DETECTION ──
        if "approved" in status or "success" in status or status == "true":
            status_ui = "Aᴘᴘʀᴏᴠᴇᴅ ✅"
        else:
            status_ui = "Dᴇᴄʟɪɴᴇᴅ ❌"
            
        # Clean raw response (remove URLs)
        raw = re.sub(r'https?://\S+', '', message).strip()
        ...
        # Escape raw response to prevent HTML crashes
        safe_response = escape(raw)
